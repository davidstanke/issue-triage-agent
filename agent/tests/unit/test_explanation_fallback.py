...
def mock_agent_app(monkeypatch):
    ...
    with patch("vertexai.init"), patch("app.agent_runtime_app.google_cloud_logging.Client"):
...
def test_query_explanation_present_and_valid(mock_agent_app):
    ...
    with patch.object(mock_agent_app, "stream_query", return_value=[mock_event]), \
         patch("app.agent_runtime_app.Event.model_validate", return_value=MockEvent(raw_response)):
